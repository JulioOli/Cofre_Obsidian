...
**O que faz:** lê um **CSV ou Excel** com **~={blue}centro de custo analítico=~** e ~={orange}**valor**=~ (~={blue}`n4_CC`=~ + ~={orange}`valor_nf`=~, **ou** ~={blue}`n4_cod_centro_custo`=~ + ~={orange}`valor_nf`=~), monta a hierarquia **n1–n4** com nomes do `02-Referencias/SAGI/sagi_rel_centro_custo.csv`, preenche metadados de fechamento (conta, filial, datas, credor etc.) e grava um **Excel** no layout da folha de fechamento.

**Entrada padrão (folha):** `02-Referencias/FOPA/base_R.csv` — colunas `n4_CC` e `valor_nf`, separador `;`.

**Saída padrão (folha):** `02-Referencias/FOPA/base_R_fechamento_folha_pagamento.xlsx` (aba `Fechamento`).
...
**Layout das colunas:** por padrão usa `02-Referencias/FOPA/base_R_fechamento_folha_pagamento_correto.xlsx` como modelo de colunas, **se o arquivo existir**; caso contrário, usa `02-Referencias/Fechamento/FECHAMENTO_ODBC_2026_04.xlsx`. Pode forçar com `--layout`.
...
```powershell
.\.venv\Scripts\python.exe 04-Notebooks\Utitlities\base_r_para_fechamento_folha.py "02-Referencias/Folha-R/7.1.22 - combustivel - diesel.xlsx"
```
...
| `--input`, `-i` | `02-Referencias/FOPA/base_R.csv` | Mesmo que o posicional; sobrescreve se os dois forem informados |
| `--output`, `-o` | *automático* | Folha: `base_R_fechamento_folha_pagamento.xlsx`; demais: `<entrada>_fechamento.xlsx` |
| `--cc-sagi` | `02-Referencias/SAGI/sagi_rel_centro_custo.csv` | Relação SAGI de CC (descrições por código) |
...
.\.venv\Scripts\python.exe 04-Notebooks\Utitlities\base_r_para_fechamento_folha.py `
  ...
  --titulo FOPA_05_2026 `
  --output 02-Referencias/FOPA/base_R_fechamento_folha_pagamento_05.xlsx
```
...
```powershell
.\.venv\Scripts\python.exe 04-Notebooks\Utitlities\base_r_para_fechamento_folha.py `
  "02-Referencias/Folha-R/7.1.22 - combustivel - diesel.xlsx" `
...
**Saída padrão:** `02-Referencias/Folha-R/R_FOPA_fechamento.xlsx` (aba `Fechamento`).
...
| Argumento | Padrão | Função |
|-----------|--------|--------|
| `--input` | `02-Referencias/Folha-R/R_FOPA.xlsx` | Planilha de origem |
| `--output` | `02-Referencias/Folha-R/R_FOPA_fechamento.xlsx` | Arquivo gerado |
...
.\.venv\Scripts\python.exe 04-Notebooks\Utitlities\r_fopa_para_fechamento.py `
  --mes 2026-04 `
  --output 02-Referencias/Folha-R/R_FOPA_fechamento_04.xlsx
```
...
- Arquivos em `02-Referencias/`: `FOPA/base_R.csv`, `SAGI/sagi_rel_centro_custo.csv`, `FOPA/base_R_fechamento_folha_pagamento_correto.xlsx` (layout de referência).
